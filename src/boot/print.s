...
; void print(char* ax) - prints null-terminated string in ax
print:
...
_print_start:
    ...
    cmp al, 0x0     ; if al == 0
    je _print_end    ; then we're done. we assume null termination
    int 0x10        ; interrupt
    add bx, 1       ; move to the next char
    jmp _print_start ; go to the start
_print_end:
    popa            ; pop all registers
    ret             ; return from routine

; void print_line(ax) - prints null-terminated string in ax, then prints CRLF
print_line:
